...
Original file is located at
    https://colab.research.google.com/drive/1PAYt_k4bM7C_yw44djcy2N4K2ZqF3ywm
"""
...
# 1. RANDOM SEARCH
print("\n1. Starting Random Search...")

from scipy.stats import randint, uniform, loguniform
from sklearn.model_selection import RandomizedSearchCV
from catboost import CatBoostRegressor

# Ubah ke distribusi yang setara dengan ruang pencarian Bayesian
param_random = {
    'depth': randint(3, 11),  # 3 sampai 10 (inklusif batas bawah, eksklusif batas atas)
    'learning_rate': loguniform(0.001, 1.0),
    'iterations': randint(50, 501),
    'subsample': uniform(0.6, 0.4),  # uniform dari 0.6 ke 1.0
    'rsm': uniform(0.6, 0.4),        # uniform dari 0.6 ke 1.0
}

model_catboost_random = CatBoostRegressor(
    random_state=42,
    verbose=False,
    early_stopping_rounds=20
)
...
random_search = RandomizedSearchCV(
    estimator=model_catboost_random,
...
model_bayes = random_search.best_estimator_  # disamakan namanya untuk struktur tetap
best_params_bayes = random_search.best_params_
print("Best Random Search Parameters:", best_params_bayes)

print("\n=== GENERATING LEARNING CURVE FOR RANDOM SEARCH - CATBOOST ===")

def plot_learning_curve_random_cat(model_params, model_name, X_train, X_valid, y_train, y_valid):
    """Plot learning curve for Random Search optimized model (CatBoost)"""
...
# Jalankan untuk CatBoost hasil tuning Random Search
model_random_cat_detailed = plot_learning_curve_random_cat(
    best_params_bayes,  # tetap gunakan variabel ini jika belum diubah namanya
    "CatBoost - Random Search",
    X_train, X_valid,
    y_train, y_valid
)

print("\n=== MODEL EVALUATION: BASELINE VS RANDOM SEARCH (CatBoost) ===")
...
y_pred_test_random = model_bayes.predict(X_test)  # Tetap pakai model_bayes jika belum diganti nama
...
for metric in ['MSE', 'RMSE', 'MAE', 'R²']:
    ...
    opt = models_metrics['Random_Search'][metric]
    if metric == 'R²':
...
# Waktu training Random Search
print(f"\n=== TRAINING TIME (Random Search - CatBoost) ===")
print(f"Time: {(end_random - start_random) / 60:.2f} minutes")  # Ganti variabel ini jika pakai start_random, end_random

# Cek model terbaik berdasarkan RMSE
if models_metrics['Random_Search']['RMSE'] < models_metrics['Baseline']['RMSE']:
    best_method = "Random_Search"
    best_rmse = models_metrics['Random_Search']['RMSE']
else:
...
fig.suptitle('CatBoost: Baseline vs Random Search - Performance Comparison', fontsize=16)

for idx, metric in enumerate(['MSE', 'RMSE', 'MAE', 'R²']):
    row, col = divmod(idx, 2)
    values = [models_metrics['Baseline'][metric], models_metrics['Random_Search'][metric]]
    bars = axes[row, col].bar(['Baseline', 'Random_Search'], values, alpha=0.7)
