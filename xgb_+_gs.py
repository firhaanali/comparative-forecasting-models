...
Original file is located at
    https://colab.research.google.com/drive/12-mbtiEeV77DO4acAmrAyBjpkVzAr0Gk
"""
...
# 4. GRID SEARCH (Limited parameter space for computational efficiency)
print("\n4. Starting Grid Search...")
...
model_xgb_grid = xgb.XGBRegressor(
    objective='reg:squarederror',
    random_state=42,
    verbosity=0
)
...
print("\n=== GENERATING LEARNING CURVES FOR GRID SEARCH ===")
...
def plot_learning_curves(model_params, model_name, dtrain, dvalid, y_train, y_valid):
    ...
    # Remove sklearn-specific parameters
    ...
    axes[0, 1].axvline(model.best_iteration, color='r', linestyle='--')
    ...
    for i in range(1, model.best_iteration + 2):
        pred_train = model.predict(dtrain, iteration_range=(0, i))
        pred_valid = model.predict(dvalid, iteration_range=(0, i))
        mae_train.append(mean_absolute_error(y_train, pred_train))
        mae_valid.append(mean_absolute_error(y_valid, pred_valid))

    axes[1, 0].plot(mae_train, label='Train MAE')
    axes[1, 0].plot(mae_valid, label='Validation MAE')
    axes[1, 0].axvline(model.best_iteration, color='r', linestyle='--')
    ...
    for i in range(1, model.best_iteration + 2):
        ...
        r2_valid.append(r2_score(y_valid, pred_valid))

    axes[1, 1].plot(r2_train, label='Train R²')
    axes[1, 1].plot(r2_valid, label='Validation R²')
    axes[1, 1].axvline(model.best_iteration, color='r', linestyle='--')
...
# Jalankan hanya untuk Grid Search
model_grid_detailed = plot_learning_curves(best_params_grid, "Grid Search", dtrain, dvalid, y_train, y_valid)

print("\n=== MODEL EVALUATION AND COMPARISON (GRID SEARCH ONLY) ===")
...
for metric in ['MSE', 'RMSE', 'MAE', 'R²']:
    ...
    grid = models_metrics['Grid_Search'][metric]
    if metric == 'R²':
        improvement = (grid - base) / abs(base) * 100 if base != 0 else float('inf')
    else:
        improvement = (base - grid) / base * 100
...
timing_df = pd.DataFrame({
    'Method': ['Grid_Search'],
    'Time_Minutes': [(end_grid - start_grid) / 60]
}).round(2)

print("\n=== TRAINING TIME COMPARISON ===")
print(timing_df.to_string(index=False))

# Best model (dari 2 ini saja)
best_method = 'Grid_Search' if models_metrics['Grid_Search']['RMSE'] < models_metrics['Baseline']['RMSE'] else 'Baseline'
best_rmse = min(models_metrics['Grid_Search']['RMSE'], models_metrics['Baseline']['RMSE'])
...
for i, metric in enumerate(metrics):
    ...
    bars = plt.bar(['Baseline', 'Grid_Search'], values, alpha=0.7)
...
print("\n=== SUMMARY ===")
print("Grid Search vs Baseline model evaluation completed.")
print("Refer to the RMSE and training time to choose the final model.")