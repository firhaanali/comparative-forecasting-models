...
Original file is located at
    https://colab.research.google.com/drive/1SIQVzN5qTtyB97rKB2wQE-16Q7zadp20
"""
...
print("\n1. Starting Bayesian Optimization...")
param_bayes = {
...
end_bayes = time.time()
print(f"Bayesian Optimization completed in {(end_bayes - start_bayes) / 60:.2f} minutes.")

model_bayes = bayes_search.best_estimator_
best_params_bayes = bayes_search.best_params_
print("Best Bayesian Parameters:", best_params_bayes)

# === DETAILED LEARNING CURVE FOR BAYESIAN OPTIMIZATION ONLY ===
print("\n=== GENERATING LEARNING CURVE FOR BAYESIAN OPTIMIZATION ONLY ===")

def plot_learning_curve_bayesian(model_params, model_name, dtrain, dvalid, y_train, y_valid):
    """Plot learning curve for Bayesian optimized model (XGBoost)"""
    ...
    # Remove n_estimators if present (handled by num_boost_round)
    ...
    # Extract RMSE and calculate MSE
    ...
    # Generate MAE and R² over iterations
    mae_train, mae_valid = [], []
    r2_train, r2_valid = [], []
...
# Jalankan hanya untuk Bayesian Optimization
model_bayes_detailed = plot_learning_curve_bayesian(
    best_params_bayes,
...
# === MODEL EVALUATION AND COMPARISON (Bayesian Optimization Only) ===
print("\n=== MODEL EVALUATION: BASELINE VS BAYESIAN OPTIMIZATION ===")
...
y_pred_test_bayes = model_bayes.predict(X_test)
...
models_metrics = {
    ...
    'Bayesian_Opt': {
        'MSE': mean_squared_error(y_test, y_pred_test_bayes),
        'RMSE': np.sqrt(mean_squared_error(y_test, y_pred_test_bayes)),
        'MAE': mean_absolute_error(y_test, y_pred_test_bayes),
        'R²': r2_score(y_test, y_pred_test_bayes)
    }
}
...
metrics_df = pd.DataFrame({
    ...
    'Bayesian_Opt': [models_metrics['Bayesian_Opt']['MSE'], models_metrics['Bayesian_Opt']['RMSE'],
                     models_metrics['Bayesian_Opt']['MAE'], models_metrics['Bayesian_Opt']['R²']]
}).round(4)
...
for metric in ['MSE', 'RMSE', 'MAE', 'R²']:
    baseline_val = models_metrics['Baseline'][metric]
    bayes_val = models_metrics['Bayesian_Opt'][metric]

    if metric == 'R²':
        improvement = (bayes_val - baseline_val) / abs(baseline_val) * 100 if baseline_val != 0 else float('inf')
    else:
        improvement = (baseline_val - bayes_val) / baseline_val * 100
...
# Training time
training_time_bayes = (end_bayes - start_bayes) / 60
print(f"\n=== TRAINING TIME (Bayesian Optimization) ===")
print(f"Time: {training_time_bayes:.2f} minutes")

# Determine best model
if models_metrics['Bayesian_Opt']['RMSE'] < models_metrics['Baseline']['RMSE']:
    best_method = "Bayesian_Opt"
    best_rmse = models_metrics['Bayesian_Opt']['RMSE']
else:
...
for metric in ['MSE', 'RMSE', 'MAE', 'R²']:
    ...
    opt = models_metrics['Bayesian_Opt'][metric]
    if metric == 'R²':
...
for idx, metric in enumerate(['MSE', 'RMSE', 'MAE', 'R²']):
    ...
    values = [models_metrics['Baseline'][metric], models_metrics['Bayesian_Opt'][metric]]
    bars = axes[row, col].bar(['Baseline', 'Bayesian_Opt'], values, alpha=0.7)
